...
Options: `-m "message"` (skip editor), `--contrib N` / `-c N` (LLM contribution %, skips prompt), `--last-used` / `-l` (skip AI/model selection, use last-used), `--help` (show usage). All other args pass through to `git commit`.
With `-m "..."` and `--last-used`/`-l`, the command is fully non-interactive.
